...
HOST void Dvc_UncNavigationState::CopyToGPU(Dvc_UncNavigationState* Dvc, int scenarioID, const UncNavigationState* Hst, bool copy_cells)
{
	...
	if(copy_cells)
	{
		...
		if(Globals::config.use_multi_thread_ && cuda_streams)
		{
...
HOST void Dvc_UncNavigationState::ReadBackToCPU(const Dvc_UncNavigationState* Dvc, UncNavigationState* Hst, bool copy_cells)
{
	...
	HANDLE_ERROR(cudaMemcpy((void*)tmp[ThreadID], (const void*)Dvc, sizeof(Dvc_UncNavigationState), cudaMemcpyDeviceToHost));

	Hst->rob.x=tmp[ThreadID]->rob.x; Hst->rob.y=tmp[ThreadID]->rob.y;
	Hst->weight=tmp[ThreadID]->weight;
	if(copy_cells)
	{
		Hst->goal.x=tmp[ThreadID]->goal.x;Hst->goal.y=tmp[ThreadID]->goal.y;
		//Hst->allocated_=tmp[ThreadID]->allocated_;
		Hst->state_id=tmp[ThreadID]->state_id;
		Hst->scenario_id=tmp[ThreadID]->scenario_id;
		Hst->sizeX_=tmp[ThreadID]->sizeX_; Hst->sizeY_=tmp[ThreadID]->sizeY_;
	}
}
HOST void Dvc_UncNavigationState::ReadBackToCPU2(const Dvc_UncNavigationState* Dvc,std::vector<State*> Hst, bool copy_cells)
{
	...
	if(NumParticles!=0)
	{
		if(copy_cells)
		{
			...
			if(Globals::config.use_multi_thread_ && cuda_streams)
			{
				CopyCells_back<<<grid1, threads1, 0, ((cudaStream_t*)cuda_streams)[ThreadID]>>>
			...
			for(int i=0;i<NumParticles;i++)
			{
				...
				//nav_state->InitCells(nav_state->sizeX_, nav_state->sizeY_);
				if(Globals::config.use_multi_thread_ && cuda_streams)
				{
...
void BaseUncNavigation::CreateMemoryPool(int mode) const
{
	...
	{
	case 0:
		if(cell_memory_pool_==NULL && Globals::config.use_multi_thread_)
			cell_memory_pool_=new Array_MemoryPool<bool>(Globals::config.use_multi_thread_);
		break;
	case 1:
		if(cell_memory_pool_ && Globals::config.use_multi_thread_){ cell_memory_pool_->ResetChuncks();}; break;
	}
*/
...
void BaseUncNavigation::DestroyMemoryPool(int mode) const
{
	...
	{
	case 0:
		if(cell_memory_pool_ && Globals::config.use_multi_thread_){delete cell_memory_pool_;cell_memory_pool_=NULL;}; break;
	case 1:
		if(cell_memory_pool_&& Globals::config.use_multi_thread_){ cell_memory_pool_->ResetChuncks();}; break;
	}*/
}
...
{//numParticles==num_Scenarios
	...
	case 0:
		...
		tmp=new Dvc_UncNavigationState*[num_threads];
		...
		for(int i=0;i<num_threads;i++)
		{
			...
			HANDLE_ERROR(cudaHostAlloc((void**)&tmp[i],1*sizeof(Dvc_UncNavigationState),0));
	...
	case 2:
		...
		Dvc_UncNavigationState* tmp_list=gpu_memory_pool_->Allocate(numParticles);
		...
		void* CUDAstream, int shift) const
{
	...
	if(CUDAstream)
	{
		CopyParticles<<<grid, threads,0, *(cudaStream_t*)CUDAstream>>>(static_cast<Dvc_UncNavigationState*>(des),
				static_cast<Dvc_UncNavigationState*>(src)+src_offset,tmp_result[(ThreadID+shift)%Globals::config.NUM_THREADS],
				IDs,num_particles, streams,stream_pos);
		//HANDLE_ERROR(cudaMemcpyAsync(temp_weight[(ThreadID+shift)%Globals::config.NUM_THREADS], tmp_result[(ThreadID+shift)%Globals::config.NUM_THREADS], sizeof(float),cudaMemcpyDeviceToHost, *(cudaStream_t*)CUDAstream));
		//if(num_particles>0)
...
float BaseUncNavigation::CopyGPUWeight2(void* cudaStream, int shift) const
{
	...
	else
	{
		//HANDLE_ERROR(cudaMemcpyAsync(temp_weight[ThreadID], tmp_result[ThreadID], sizeof(float),cudaMemcpyDeviceToHost, *(cudaStream_t*)cudaStream));
		HANDLE_ERROR(cudaStreamSynchronize(*(cudaStream_t*)cudaStream));
		if(*temp_weight[(ThreadID+shift)%Globals::config.NUM_THREADS]>1)
			Global_print_value(this_thread::get_id(),*temp_weight[(ThreadID+shift)%Globals::config.NUM_THREADS],"Wrong weight from CopyGPUWeight");
		return *temp_weight[(ThreadID+shift)%Globals::config.NUM_THREADS];
	}
}
