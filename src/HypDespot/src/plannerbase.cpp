...
option::Descriptor* BuildUsage(string lower_bounds_str,
	...
	static option::Descriptor usage[] =
			{ { E_HELP, 0, "", "help", option::Arg::None,
					...
					{ E_NUMOBS, 0, "", "nobs",
												option::Arg::Required,
												"  \t--nobs <arg>  \tNumber of obs (default number of particles)." },
					...
					{ E_USE_KERAS, 0, "", "USEKERAS", option::Arg::Required,
...
void PlannerBase::OptionParse(option::Option *options, int &num_runs,
	...
	if (options[E_CPU])
	{
		...
		cout<<"use CPU multi_threading: "<<Globals::config.use_multi_thread_<<endl;
	}
	...
	{
		cout<<"No option to use Keras Model: "<<Globals::config.use_keras_model<<endl;
	}
